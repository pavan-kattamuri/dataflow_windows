...
class PrintElements(beam.DoFn):
    ...
    def process(self, element, window=beam.DoFn.WindowParam):
        ...
        for row in elements:
            print(f"serverID - {serverID}, window - {window_start},{window_end}, timestamp -  {row['timestamp']}, CPU_Utilization - {row['CPU_Utilization']}")
...
def run(argv=None):
    ...
    events = p | 'Create Events' >> beam.Create(data) \
        ...
        | 'Add Window info' >>  beam.WindowInto(beam.window.SlidingWindows(5, 4)) \
